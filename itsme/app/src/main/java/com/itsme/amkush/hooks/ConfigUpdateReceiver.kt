package com.itsme.amkush.hooks
   ...
   * updates AppState, and calls [DecoderLauncher.restart] to swap the source
   * on the fly.
   *
   * Works in all environments:
   *   • Rooted (LSPosed) — standard cross-process broadcast delivery.
   *   • Virtual cloners (Mochi Cloner, VirtualXposed, Parallel Space) — both
   *     apps live in the same virtual sandbox so broadcasts are delivered
   *     within the cloner's process space without any root requirement.
   *
  ...
  class ConfigUpdateReceiver : BroadcastReceiver() {
      ...
      override fun onReceive(context: Context, intent: Intent) {
          ...
          if (!active) {
              ...
              DecoderLauncher.stop()
              return
          }

          AppState.isHookingActive = true
          DecoderLauncher.restart(streamUrl, mediaUri)
      }
  }
