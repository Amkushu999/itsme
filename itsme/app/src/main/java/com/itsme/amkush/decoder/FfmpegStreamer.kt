...
/**
 * Network/file stream decoder for the Xposed injection pipeline.
 *
 * WHY NOT FFmpeg Kit here:
 *   This object's code executes inside the HOOKED TARGET APP's process (loaded via
 *   Xposed/LSPosed). FFmpeg Kit relies on native .so libraries that live in the
 *   MODULE's own lib directory. The target app's process cannot resolve them, so any
 *   call to FFmpegKit would throw UnsatisfiedLinkError at runtime.
 *
 *   Solution: FFmpeg Kit is used in the MODULE's own process only — see
 *   StreamPreviewDialog for the user-facing stream player.  The injection decoder
 *   here uses Android's built-in MediaExtractor + MediaCodec, which work in every
 *   process without extra native libs.
 *
 * Guarantees:
 ...
 *  - RTSP streams use TCP transport via MediaExtractor headers (avoids UDP packet loss).
 */
object FfmpegStreamer {
    ...
    private fun decodeSession(url: String) {
        ...
        try {
            // Pass RTSP-over-TCP hint so we don't lose packets on poor networks.
            // For non-RTSP URLs the extra header is harmless.
            val headers = mapOf(
                "rtsp_transport" to "tcp",
                "timeout"        to "5000000"  // 5 s connect timeout (µs)
            )
            ...
            reader = android.media.ImageReader.newInstance(width, height, ImageFormat.YUV_420_888, 2)
