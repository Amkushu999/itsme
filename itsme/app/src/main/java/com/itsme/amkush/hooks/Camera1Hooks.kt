...
/**
 * Camera1Hooks — GStreamer/physical-camera-block architecture
 *
 ...
 *     → Route it to InjectionService (GStreamer) via GStreamerSurfaceRouter
 *
 *   startPreview()
 *     → Start a 30-fps PreviewCallback heartbeat if callback was registered
 *     → GStreamer starts pushing frames to the surface in the module process
 *
 *   stopPreview() / release()
 *     → Stop heartbeat, notify GStreamer to stop
 *
 *   takePicture(shutter, raw, jpeg)
 *     → Intercept jpeg callback, return a JPEG of the current GStreamer frame
 *     → The JPEG is synthesized from AppState.currentFrame (set by FfmpegStreamer
 *       fallback or by GStreamer appsink callback forwarded over SharedPrefs/IPC)
 *
 ...
 * The GStreamer pipeline fills AppState.currentFrame in the module process and
...
object Camera1Hooks {
            ...
            XposedHelpers.findAndHookMethod(camClass, "setPreviewDisplay", SurfaceHolder::class.java,
                object : XC_MethodHook() {
                    override fun beforeHookedMethod(param: MethodHookParam) {
                        ...
                        routeCamera1Surface(fake, surface, state)
                    }
                }
            )
    ...
    private fun routeCamera1Surface(fake: Any, surface: Surface, state: Cam1State) {
        ...
        GStreamerSurfaceRouter.routeSurfaces(
            fake,
            listOf(surface),
     ...
     * When a Surface is used, GStreamer pushes directly via ImageWriter.
     */
            ...
            XposedHelpers.findAndHookMethod(camClass, "release",
                object : XC_MethodHook() {
                    override fun beforeHookedMethod(param: MethodHookParam) {
                        ...
                        if (state != null) GStreamerSurfaceRouter.stopSession(fake)
                        stopPreviewHeartbeat(fake)
